...
#    system("module load eclipidrmergepeaks");
    system("idr --samples $file1_bed $file2_bed --input-file-type bed --rank 5 --peak-merge-method max --plot -o $outfi");
}
my %idr_output;
&parse_outfi($outfi);
...
#    print STDERR "running fix_file on $fi\n";
	...
	print OUT "".$tmp[0]."\t".$tmp[1]."\t".$tmp[2]."\t".$tmp[11]."\t".sprintf("%.10f",$tmp[$#tmp])."\t".$str."\n";
